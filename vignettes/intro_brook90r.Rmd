...
```{r,echo = 1:3, results = 'hide',fig.height=4, fig.width=7}
...
```
In the latter case, the yearly values are interpreted to be valid for January 1st of the respective years, and additional parameters `height.end`, `sai.end`, `densef.end` have to be provided to `Run.B90`. 
defining the values at the end of the simulation, to which the value of the last year is interpolated to.

The yearly values for `height`, `sai`, and `densef` are  currently interpreted to be valid for the 1st of January of the respective years of the simulations. Between these dates, the values are interpolated to derive a daily timeseries. For the last year of the simulation, the values are extrapolated using the daily increment of the year before the last year.

Another option for incorporating between-year variation of plant properties is to provide a table of values via the `longtermdev`-argument of `Run.B90`. To take effect, the option `options.b90$longtermdyn` has to be set to 'table', so that the values passed via parameters are ignored. We use the table `standprop_slb1`, that contains the observed stand data of Solling Beech Experimental site from 1966 to 2014.
```{r,echo = 1:3, results = 'hide',fig.height=4, fig.width=7}
options.b90$standprop <- 'table'
options.b90$standprop.interp <- "linear"
b90.results.slb1.lai <- Run.B90(project.dir = "/example_run_b90/",
                            options.b90 = options.b90,
                            param.b90 = param.b90,
                            climate = meteo_slb1,
                            soil = soil,
                            longtermdev = standprop_slb1,
                            outputmat = output,
                            output.log = "b90.log",
                            path_b90.exe = "H:/R-Packages/brook90r/b90.exe")
```
