...
Another option for incorporating between-year variation of plant properties is to provide a table of values via the parameter `standprop.table` of the `param.b90`-argument of `Run.B90`. To take effect, the option `options.b90$standprop.input` has to be set to 'table'. In this case, the values passed via parameters `maxlai`, `height`, `sai`, `densef`, and `age.ini` are ignored.
For demonstration purposes we use the table `standprop_slb1`, that contains the observed stand data of the Solling Beech Experimental site from 1966 to 2014, along with estimated leaf area and stem area index using allometric function.
```{r,echo = 1:3, results = 'hide',fig.height=4, fig.width=7}

#Extend simulation period
options.b90$startdate <- as.Date("1995-01-01")
options.b90$enddate <- as.Date("2005-12-31")
