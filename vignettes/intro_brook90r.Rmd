...
b90.results.slb1 <- Run.B90(project.dir = "example_run_b90/",
                            ...
                            outputmat = output,
                            output.log = "b90.log",
                            path_b90.exe = "D:/Arbeit/B90/b90.exe")
```
...
sai_c <- approx_standprop(x.years=simyears,
                                 y = param.b90$sai,
                                 approx.method = options.b90$standprop.interp)

# linear interpolation       
options.b90$standprop.interp <- 'linear'
param.b90$height.end <- 21.6
param.b90$sai.end <- 0.5
height_l <- approx_standprop(x.years=c(simyears, max(simyears) + 1),
                             y = c(param.b90$height, param.b90$height.end),
                             xout.years = simyears,
                             approx.method = options.b90$standprop.interp)
sai_l <- approx_standprop(x.years=c(simyears, max(simyears) + 1),
...
For demonstration purposes we use the table `standprop_slb1`, that contains observed stand data of the Solling Beech Experimental site from 1966 to 2014, along with estimated leaf and stem area index from allometric functions.
...
A third option generates a linear root density depth distriution, with the maximum at the uppermost soil layer and a root density of 0 at 'maxrootdepth'. If the parameter 'relrootden' is provided, the first element of the vector is used as the maximum, otherwise the intepolation is made between 0 and 1. The last option returns a uniform root distribution, with the first vector-element of 'relrootden' (if provided) as value for all layers down to 'maxrootdepth'.
