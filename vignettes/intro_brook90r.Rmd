...
```
Theoretically, the root density estimated with the beta-model approaches but never reaches a value of 0 in deep soil layers. Therefore, the parameter `maxrootdepth` was defined, setting the root density to zero at the specified depth. The parameter `humusroots` defines, if humus layers should have roots or not....
...
plot(c(roots_beta[1],roots_beta),c(0,soil_slb1$lower), type = 's')
abline(h=0)
